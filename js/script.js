...
let objCalculator = { 
    //  Declaring the object global variables required for the functionality to work
    number1: '',
    number2: '',
    operator: '',
    blnEquals: false,
    ...
    //  This function is called to initialize the object and prepare all variables 
    // that will need to exist for the object to run
    init:function() {
        this.getAllElements();
    ...
    //  This function will add event listeners directly to singular HTML objects or loop through an array of 
    //  HTML objects to attach event listeners. 
    addEventListeners:function(){
        //  Because 'this' represents the current object (objCalculator) when we attach an event listener to a HTML object 'this' ends up 
        //  referencing the HTML object and not the current object so we are setting this variable so that we can pass objCalculator to the 
        //  function being ran inside of the event listener. 
        let _self = this;
        ...
        //  Looping through all objects in arrOperator to add the same event listener to them.
    ...
    //  this funciton is used to build up the data required to be displayed in the sum preview area of the calculator
    preview:function(data){
        // console.log(data); 
        //  if a successful equals function has been ran and a new preview needs to be displayed, 
        //  clear all data that is currently set and unset blnEqualsSuccess
        ...
        if(dataType == 'number'){
            ...
            if(dataType == 'decimal'){
                ...
                if(this.number1){
                    //  if number2 has a value then complete the sum so far and update the display otherwise overwrite the operator
                    //  with a new operator 
                    if(this.number2){
        ...
        hasDecimal:function(number){
            ...
            switch(number){
                case 'number1':
                    ...
                    //  the reason why we set a negative result as -1 is because the character could be at position 0 and 0 would render
                    //  false
    ...
    //     set the value of the completed sum to number1
       this.number1 = sum;
    //     update operator to the operator passed
...
equals:function(){
    ...
    if(blnCanDoMaths){
        ...
        if(sum !== false){
            ...
            //  if the equals button was pressed, the displays need to output differently to if a number, operator or decimal 
            //  preview function called the equals function.
            if(this.blnEquals){
                let strPreviousSum = this.objPreview.value;
                this.objPrevious.value = strPreviousSum;
                this.objPreview.value = "";
...
outputError:function(strErrorMessage){
    ...
    //  add a timeout function to hide the error message after 2500 milliseconds
